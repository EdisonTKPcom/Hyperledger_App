...
app.get("/cars", async (req, res) => {
  try {
    ...
    if (!userExists) {
      res.json({
        status: false,
        error: { message: "User not exist in the wallet" },
      });
      return;
    }
    ...
    // res.json({ status: true, cars: network.toString() });
...
app.post("/cars", async (req, res) => {
  ...
  try {
    ...
    res.json({
      status: true,
      message: "Transaction (create car) has been submitted.",
    });
  } catch (err) {
    res.json({ status: false, error: err });
  }
...
app.put("/cars", async (req, res) => {
  ...
  try {
    ...
    res.json({
      status: true,
      message: "Transaction (change car owner) has been submitted.",
    });
  } catch (err) {
    res.json({ status: false, error: err });
  }
